                ...
                Ok(Event::Start(ref e)) | Ok(Event::Empty(ref e)) => {
                    ...
                    match local_name {
                        ...
                        "trans-unit" => {
                            ...
                            let max_width =
                                get_attr(e, b"maxwidth").and_then(|v| v.parse::<u32>().ok());
                        ...
                        "alt-trans" => {
                            ...
                            let match_quality =
                                get_attr(e, b"match-quality").and_then(|v| v.parse::<f32>().ok());
                    ...
                    match local_name {
                        ...
                        "trans-unit" => {
                            if let Some(tu) = current_tu.take() {
                                ...
                                let alternatives: Vec<AlternativeTranslation> = tu
                                    .alternatives
                                    .iter()
                                    .map(|a| AlternativeTranslation {
    ...
    fn write(&self, resource: &I18nResource) -> Result<Vec<u8>, WriteError> {
        ...
        let (datatype, original) = match &resource.metadata.format_ext {
            Some(FormatExtension::Xliff1(ext)) => {
                (ext.datatype.as_deref(), ext.original.as_deref())
            }
            _ => (None, None),
        };
    ...
    fn write_trans_unit(
        ...
        if !entry.contexts.is_empty() {
            ...
            for (purpose, ctxs) in &groups {
                ...
                writer
                    .write_event(Event::Start(cg_start))
                    .map_err(|e| WriteError::Serialization(e.to_string()))?;
                ...
                for ctx in ctxs {
                    ...
                    writer
                        .write_event(Event::Start(ctx_start))
                        .map_err(|e| WriteError::Serialization(e.to_string()))?;
                    writer
                        .write_event(Event::Text(BytesText::new(&ctx.value)))
                        .map_err(|e| WriteError::Serialization(e.to_string()))?;
                    writer
                ...
                writer
                    .write_event(Event::End(BytesEnd::new("context-group")))
                    .map_err(|e| WriteError::Serialization(e.to_string()))?;
            }
        }
        ...
        for alt in &entry.alternatives {
            ...
            writer
                .write_event(Event::Start(alt_start))
                .map_err(|e| WriteError::Serialization(e.to_string()))?;
            ...
            writer
                ...
                .map_err(|e| WriteError::Serialization(e.to_string()))?;
            writer
                .write_event(Event::Text(BytesText::new(&alt.value)))
                .map_err(|e| WriteError::Serialization(e.to_string()))?;
            writer
                .write_event(Event::End(BytesEnd::new("target")))
                .map_err(|e| WriteError::Serialization(e.to_string()))?;
            ...
            writer
                .write_event(Event::End(BytesEnd::new("alt-trans")))
                .map_err(|e| WriteError::Serialization(e.to_string()))?;
        }
